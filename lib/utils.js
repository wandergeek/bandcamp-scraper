...
var parseSoundID = function(body,soundURL) {
    var soundID = "";
    var myLines = body.split("\n");
    var soundIDLine = myLines[myLines.length - 2];
    // var rePattern= new RegExp(/[0-9]+/);
    var rePattern= new RegExp(/<!-- [a-z]+ id ([0-9]+) -->/);
    var matches = body.match(rePattern);
...
//this hacky function scrubs out bad results
var scrubResultsForTarget = function(collection) {
  var out = [];
  //TODO: this is a hack. results tend to be good if first result is an artist
  if (collection[0]["type"] != "artist" ) {
    console.log("scrubber returning empty list");
    return [];
  }

//TODO: This sucks, but bandcamp results are too noisy atm
  var bandLocation = collection[0]["from"].toLowerCase();
  if(bandLocation.indexOf("australia") == -1) {
    console.log(bandLocation,"did not contain the word australia, returning empty");
    return [];
  }
    ...
    for(var i=0; i<collection.length-1; i++) {
      ...
      if(entry['type'] == 'track' || entry['type'] == 'album') {
        // console.log("got track/album:", entry['artist'],":", artist['name']);
        if(entry['artist'].toLowerCase() == artist['name'].toLowerCase()) {
          // console.log('scrubber: pushing entry ', entry['artist'], artist['name']);
          out.push(entry);
        }
      }
    }
...
module.exports.parseSoundID = parseSoundID;
module.exports.scrubResultsForTarget = scrubResultsForTarget;
